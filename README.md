...
- **C++ 重写**：全栈 C++17 + Qt 6，比 Python V1 启动快 10×、内存占用低 60% / **Full C++ rewrite** — C++17 + Qt 6; 10× faster startup, 60% less memory than Python V1
- **GPU 硬件加速**：Qt Quick Scene Graph（OpenGL/Vulkan 自动选择），60fps 高分辨率稳定渲染 / **GPU acceleration** — Qt Quick Scene Graph (auto OpenGL/Vulkan), steady 60 fps at high resolution
- **音频响应可视化**：柱状 / 镜像频谱、粒子波纹、封面随节拍脉冲 / **Audio-reactive visuals** — bar / mirror spectrum, particle ripple, cover pulse on beat
- **待机 LOGO 自定义**：支持 PNG/JPG/GIF（透明通道保留、自动循环动画）/ **Custom standby logo** — PNG/JPG/GIF (alpha preserved, animated GIF loops automatically)
- **背景媒体自定义**：纯色（可选颜色）/ GIF / WEBP / MP4 / MOV / MKV，自动 PreserveAspectCrop（填满屏幕、裁剪边缘无黑边）/ **Custom background** — solid colour (pick any), GIF, WEBP, MP4, MOV, MKV; PreserveAspectCrop (edge-cropped, no black bars)
- **黑色遮罩深度**：自定义背景上盖 0–100% 黑色半透明遮罩，平衡背景与前景对比度 / **Black overlay depth** — 0–100% black semi-transparent overlay on custom backgrounds
- **多显示器 / 任意屏幕**：默认窗口化，拖到任意屏幕按 **F** 全屏 / **Any display** — windowed by default, drag anywhere and press **F** for fullscreen
- **无 DB 也能跑**：没有指纹库时仅渲染频谱与波形（standby-only 模式）/ **DB optional** — spectrum + ripple only (standby mode) when no fingerprint DB
- **中英双语界面**：一键切换 / **Bilingual UI** — one-click switch 中文 / English
- **便携部署**：exe + Qt 运行库 + 空 `data/` 文件夹拷走即用 / **Portable** — exe + Qt runtime + empty `data/` folder; fingerprints and settings auto-generated on first run
...
从 [GitHub Releases](https://github.com/ichiryu0021/VJVision/releases) 下载最新版压缩包，解压到任意位置。
Download the latest release from [GitHub Releases](https://github.com/ichiryu0021/VJVision/releases) and extract it anywhere.
...
> Copy the whole `VJVision/` folder (exe + dll + qml + data/) to a USB stick or another machine — settings and fingerprints carry over. Only the audio-device index may need re-picking on a different machine.
...
| 视觉效果 | 待机 LOGO（浏览 + 清除）、背景来源（默认/自定义）、底色/背景媒体、黑色遮罩深度 / Standby logo, background source (default/custom), colour or media picker, black overlay depth |
...
1. Delete `VJVision_prefs.json` next to the exe to regenerate defaults
2. Confirm Qt runtime is fully deployed (run windeployqt)
3. Confirm GPU driver supports OpenGL 3.0+
