        ...
        */
        ...
        Canvas {
            ...
            onPaint: {
                ...
                var alphaArr = [0.75, 0.65, 0.55]

                // HARD-CODED distinct colors
                var layerColors = [
                    Qt.rgba(0.88, 0.92, 1.00, 1),   // top — bright white-blue
                    Qt.rgba(1.00, 0.70, 0.40, 1),   // mid — warm orange
                    Qt.rgba(0.45, 0.90, 0.75, 1)    // bottom — teal green
                ]

                // PAINTER'S ALGORITHM: draw BOTTOM layer first (2), MIDDLE (1), TOP last (0).
                // Each layer fills its wave area from the wave CURVE down to h.
                // Higher layers naturally cover lower ones where they overlap.
                for (var li = 2; li >= 0; --li) {
                    ...
                    var grad = ctx.createLinearGradient(0, yOffArr[li] - ampArr[li], 0, h)
                    grad.addColorStop(0.00, Qt.rgba(r, g, b, a))
                    grad.addColorStop(1.00, Qt.rgba(r, g, b, 0.0))
                    ctx.fillStyle = grad
                    ctx.fill()
                }
            }
