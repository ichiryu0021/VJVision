        ...
        */
        ...
        Canvas {
            ...
            onPaint: {
                ...
                // Colors from C++ cover sampler (top-2 dominant + 1 contrast)
                var colors = mx.trackColors
                if (!colors || colors.length < 3) colors = [Qt.hsla(0.6,0.5,0.4,1), Qt.hsla(0.3,0.5,0.4,1), Qt.hsla(0.9,0.5,0.4,1)]
                function layerColor(layer) {
                    if (layer < colors.length) return colors[layer]
                    return Qt.hsla((root.trackHue + layer * 60) / 360, 0.5, 0.4, 0.2)
                }

                // Base: dark tint from primary cover color
                var c0 = layerColor(0)
                ctx.fillStyle = Qt.rgba(c0.r / 255 * 0.15, c0.g / 255 * 0.15, c0.b / 255 * 0.18, 1.0)
